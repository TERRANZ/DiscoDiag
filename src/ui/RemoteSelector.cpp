...
void RemoteSelector::serviceDiscovered(
    const QBluetoothServiceInfo &serviceInfo) {
  qDebug() << "Discovered service on" << serviceInfo.device().name()
  ...
  qDebug() << "Adding to list remote name " << remoteName
           << " with serviceName " << serviceName;

  auto *item = new QListWidgetItem(
      QString::fromLatin1("%1 %2").arg(remoteName, serviceName));
